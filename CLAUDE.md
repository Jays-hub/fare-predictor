...
- `primp` chrome impersonation falls back to "random" (profiles not compiled in 1.3.1). Does **not** corrupt data — only lowers hit rate, and the fallback is **normal on the x86 runner too**, not an M-series-only symptom. Bot-detection blocking is **probabilistic/intermittent**.
- Blocking shows up as one of **two `RuntimeError` flavors, both handled by retry — neither is a bug or a `fast-flights` break**: the `"Loading results"` stub, and `"No flights found"` where the response is **HTTP 200 but the body is the Google Flights homepage** (a decoy) instead of results. Tell blocking from a real break by: 200 + homepage body, and at least one combo still parsing normally (a structural change would instead crash in `parse_result`). A whole cycle can be ~90%+ blocked (e.g. 06-11: 14/15 combos lost); the freshness monitor is the safety net. Stay hobby-scale — watch a few cycles, then `pip install -U fast-flights`; no proxies/CAPTCHA evasion.
...
- **Reliability layer** (`fetch_with_retry`): wraps `get_flights`; retries on **any raised exception** (known cases: RuntimeError "Loading results" stub or "No flights found" homepage-decoy, AssertionError non-200) AND soft degraded-parse; 4 attempts, exponential backoff + jitter; returns `None` on total failure (never raises) so the loop continues.
- **Second pass**: combos that fail the first pass retried once after a 30–60s cooldown to decorrelate from transient blocking.
- **GitHub runner > local Mac** for collection — historically higher success than the M-series laptop (the `primp` impersonation still falls back to "random" there, so the edge is IP/environment, not impersonation). Note this can invert during a heavy-blocking streak on the runner's shared CI IPs.
...
- **Status**: collecting since 2026-06-04 (~2.5k rows as of 06-11); scheduled-run DB writes verified at both log and database level. Pipeline is unattended, self-guarding (expiry skip + horizon alarm), and healthy. **EDA run 2026-06-11 — 3 theses validated; next build step is the eval harness.**
