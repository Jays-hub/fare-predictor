...
- `scripts/check_deals.py` — mistake-fare alarm: pure `find_deals()` (≥20% AND ≥$20 below 14-day trailing min, ≥3 prior snapshots, latest cycle only) + `check()` that exits 1 on a deal to fire the email (inverted fail→email semantics).
...
- `.github/workflows/deals.yml` — mistake-fare alarm ~30 min after each collection (10:30/22:30 UTC); a failed run → email, but here failure = **deal found** (inverted vs. the freshness monitor).
...
- **✅ DONE (2026-06-12) — mistake-fare alarm (`scripts/check_deals.py` + `.github/workflows/deals.yml`).** Pure `find_deals()` detector: a live fare ≥**20% AND ≥$20** below its **14-day trailing min**, requiring ≥3 prior snapshots (no baseline ⇒ stay quiet); judges only the latest cycle (alert on a buyable price, never a historical deal). Reuses the fail→email channel with **inverted semantics** (exit 1 = deal found = email; silence = healthy) — documented in-file so the non-zero exit isn't "fixed." Scheduled 10:30/22:30 UTC (~30 min after collection). First real-panel run: 0 deals, exit 0.
