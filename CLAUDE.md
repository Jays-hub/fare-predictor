...
- `tests/` — pytest suite (27 tests) for the parsing layer, reshaping logic, harness dollar-accounting, and deal detection — all on synthetic data (no network/DB); `.github/workflows/test.yml` runs it on every push.
...
- **Next**: first model (**LightGBM**, scored in the harness against the baselines above — no XGBoost bake-off, redundant at this scale; features: competing-carrier price, days-the-min-held, upward-step size/frequency, lags, rolling stats, holiday flags, `price_level`) → advanced (**survival/hazard on fare-step events** — directly models thesis C; GRU/LSTM/TFT only if the panel ever grows ~10×) → decision layer.
