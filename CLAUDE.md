...
- **Next**: add the **buy-iff-Google-says-low** baseline (needs `price_level` threaded through `carrier_min_series` + a first `low` verdict observed) → first model (**LightGBM**, scored in the harness against the baselines above — no XGBoost bake-off, redundant at this scale; features: competing-carrier price, days-the-min-held, upward-step size/frequency, lags, rolling stats, holiday flags, `price_level`) → advanced (**survival/hazard on fare-step events** — directly models thesis C; GRU/LSTM/TFT only if the panel ever grows ~10×) → decision layer.
